...
class CameraOperation():
    ...
    def __init__(self, obj_cam, st_device_list, n_connect_num=0, b_open_device=False, b_start_grabbing=False,
                 ...
                 exposure_time=0, gain=0, burst_num=0, bmp_temp_name='', burst_mask_num=0):
        ...
        self.bmp_temp_name = bmp_temp_name
        self.burst_mask_num = burst_mask_num
        self.meas_count = 0
    ...
    def Trigger_once(self, nCommand):
        if True == self.b_open_device:
            if 1 == nCommand:
                self.obj_cam.MV_CC_SetEnumValue("TriggerSource", 7)
                ret = self.obj_cam.MV_CC_SetCommandValue("TriggerSoftware")
                self.obj_cam.MV_CC_SetEnumValue("TriggerSource", 0)
                if ret != 0:
    ...
    def Get_parameter(self):
        if True == self.b_open_device:
            ...
            stIntParam_offsetx = MVCC_INTVALUE()
            memset(byref(stIntParam_offsetx), 0, sizeof(MVCC_INTVALUE))
            stIntParam_offsety = MVCC_INTVALUE()
            memset(byref(stIntParam_offsety), 0, sizeof(MVCC_INTVALUE))
            stIntParam_width = MVCC_INTVALUE()
            memset(byref(stIntParam_width), 0, sizeof(MVCC_INTVALUE))
            stIntParam_height = MVCC_INTVALUE()
            memset(byref(stIntParam_height), 0, sizeof(MVCC_INTVALUE))
            ...
            ret = self.obj_cam.MV_CC_GetIntValue("OffsetX", stIntParam_offsetx)
            if ret != 0:
                tkinter.messagebox.showerror('show error', 'get offsetx fail! ret = ' + self.To_hex_str(ret))
            self.offsetx = stIntParam_offsetx.nCurValue
            ret = self.obj_cam.MV_CC_GetIntValue("OffsetY", stIntParam_offsety)
            if ret != 0:
                tkinter.messagebox.showerror('show error', 'get offsety fail! ret = ' + self.To_hex_str(ret))
            self.offsety = stIntParam_offsety.nCurValue
            ret = self.obj_cam.MV_CC_GetIntValue("Width", stIntParam_width)
            if ret != 0:
                tkinter.messagebox.showerror('show error', 'get width fail! ret = ' + self.To_hex_str(ret))
            self.width = stIntParam_width.nCurValue
            ret = self.obj_cam.MV_CC_GetIntValue("Height", stIntParam_height)
            if ret != 0:
                tkinter.messagebox.showerror('show error', 'get height fail! ret = ' + self.To_hex_str(ret))
            self.height = stIntParam_height.nCurValue
            print(stIntParam_height.nMax)
            print(stIntParam_height.nMin)
            # tkinter.messagebox.showinfo('show info','get parameter success!')

    def Set_parameter(self, frameRate, exposureTime, gain, text_max_value):
        all_good = True
        if '' == frameRate or '' == exposureTime or '' == gain:
            tkinter.messagebox.showinfo('show info', 'please type in the text box !')
            return
        if True is self.b_open_device:

            if True is self.b_start_grabbing and self.b_open_device is True:
                # 退出线程
                if True is self.b_thread_closed:
                    Stop_thread(self.h_thread_handle)  ##停止取流
                    self.b_thread_closed = False
                ret = self.obj_cam.MV_CC_StopGrabbing()
                if ret != 0:
                    tkinter.messagebox.showerror('show error', 'stop grabbing fail! ret = ' + self.To_hex_str(ret))
            ...
            ret = self.obj_cam.MV_CC_SetIntValue("Height", int(self.height))
            if ret != 0:
                tkinter.messagebox.showerror('show error',
                                             'set Height fail! ret = ' + self.To_hex_str(ret))
                all_good = False

            ret = self.obj_cam.MV_CC_SetIntValue("Width", int(self.width))
            if ret != 0:
                tkinter.messagebox.showerror('show error',
                                             'set Width fail! ret = ' + self.To_hex_str(ret))
                all_good = False
            ret = self.obj_cam.MV_CC_SetIntValue("OffsetX", int(self.offsetx))

            if int(self.offsetx) % 32 == 0:
                if ret != 0:
                    tkinter.messagebox.showerror('show error',
                                                 'set OffsetX fail! ret = ' + self.To_hex_str(ret))
                    all_good = False
            else:
                tkinter.messagebox.showerror('show error',
                                             '水平偏移值必须为32的倍数！')
            ...
            ret = self.obj_cam.MV_CC_SetIntValue("OffsetY", int(self.offsety))
            if int(self.offsety) % 2 == 0:
                if ret != 0:
                    tkinter.messagebox.showerror('show error',
                                                 'set OffsetY fail! ret = ' + self.To_hex_str(ret))
                    all_good = False
            else:
                tkinter.messagebox.showerror('show error',
                                             '垂直偏移值必须为2的倍数！')
            ...
            self.Start_grabbing(glb_root, glb_panel)
            if all_good:
                pass
    ...
    def Work_thread(self, root, panel):
        ...
        while True:
            ...
            if 0 == ret:
                ...
                if self.burst_num < 0:
                    self.Save_Bmp(buf_cache, 1, 1)
                if self.burst_num > 0:
                    self.Save_Bmp(buf_cache, 1, 1)
                    self.burst_num -= 1
                if self.burst_mask_num > 0:
                    self.Save_Bmp(buf_cache, 2, 1)
                    self.burst_mask_num -= 1
            ...
            # 合并OpenCV到Tkinter界面中
            # print('nWidth: ', self.st_frame_info.nWidth)
            # print('nHeight: ', self.st_frame_info.nHeight)
            ...
            current_image_max = np.asarray(current_image).max()
            imgtk = CTkImage(current_image, size=(330, 330))
            panel.imgtk = imgtk
    ...
    def Save_Bmp(self, buf_cache, dir_mode=0, pop_mode=0):
        if 0 == buf_cache:
            return
        self.buf_save_image = None
        if dir_mode == 0:
            ...
            file_path = os.path.join(os.getcwd(), "Measurement",
                                     "meas_" + str(self.meas_count) + '.bmp')  # 从1开始递增
            self.meas_count += 1
        elif dir_mode == 2:
            file_path = os.path.join(os.getcwd(), "Masks_photo", "mask_" + str(self.st_frame_info.nFrameNum) + '.bmp')
        self.n_save_image_size = self.st_frame_info.nWidth * self.st_frame_info.nHeight * 3 + 2048
        if self.buf_save_image is None:
            self.buf_save_image = (c_ubyte * self.n_save_image_size)()
