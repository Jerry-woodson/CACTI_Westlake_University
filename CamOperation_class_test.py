...
class CameraOperation():
    ...
    def Start_grabbing(self, root, image_panel):
    ...
    def Work_thread(self, root, image_panel):
        ...
        while True:
            ...
            img = cv2.cvtColor(np.asarray(current_image), cv2.COLOR_RGB2BGR)  # PIL转成opencv
            histCV = cv2.calcHist([img], [0], None, [256], [0, 256])  # OpenCV 函数 cv2.calcHist
            imggray = ImageTk.PhotoImage(image=histCV, master=root)
            imgtk = ImageTk.PhotoImage(image=current_image, master=root)
            avg_gray = np.sum(histCV) / (800 * 600)
            print("avg gray:", avg_gray)
    ...
    def Save_jpg(self, buf_cache):
        ...
        file_path = "./Output/"+str(self.st_frame_info.nFrameNum) + ".jpg"
        self.n_save_image_size = self.st_frame_info.nWidth * self.st_frame_info.nHeight * 3 + 2048
    ...
    def Save_Bmp(self, buf_cache, dir_mode=0, pop_mode=0):
        ...
        if dir_mode == 0:
            ...
            file_path = os.path.join(os.getcwd(), "Measurement", "mea_" + str(self.st_frame_info.nFrameNum) + '.bmp')
        elif dir_mode == 2:
